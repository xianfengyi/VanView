...
msdx::dxStatus msdx::TestGraphController::Build( TestGraphParam & param )
{
    ...
    if (!graph_param_.video.device.empty())
    {
        ...
        status = video_source_->AddFilter(graph_builder, graph_param_.video.device);
        CHK_FAIL_RETURN(STATUS_ERR(status), status);
        status = video_source_->SetFormat(graph_param_.video.frame_width, 
            graph_param_.video.frame_height, 30);
        CHK_FAIL_RETURN(STATUS_ERR(status), status);
        ...
        status = video_inf_pin_tee_->AddFilter(graph_builder, video_source_->GetOutputPin(), L"Video Inf");
        CHK_FAIL_RETURN(STATUS_ERR(status), status);
        ...
        status = video_renderer_->AddFilter(graph_builder, 
